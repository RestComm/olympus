    ...
    $.widget("ui.chatbox", {
            ...
            boxManager: {
                ...
                emotify: function(text) {
                    var emoticons = {
                        ...
                        '<3' : 'icon_heart.gif'
                    }, url = "./resources/images/emoticons/", patterns = [], metachars = /[[\]{}()*+?.\\|^$\-,&#\s]/g;
                ...
                addMsg: function(peer, msg, messageFromPeer) {
                    ...
                    // FIXME: brainslog: removed the contact picture for now..
                    ...
                    if (appendMessage === false) { //this is not a message from the last one
                        var messageDiv = document.createElement('div');
                        var timestampDiv = document.createElement('div');
                        var contactSpan = document.createElement('div');
                        ...
                        /*
                        $(timestampDiv).addClass('row');
                        $(contactSpan).addClass('col-md-7');
                        // TODO: add strong/class
                        contactSpan.innerHTML = '<strong>'+peer+'</strong>';
                        $(timestampSpan).attr("data-livestamp", Date.now()/1000);
                        $(timestampSpan).addClass("col-md-5");
                        ...
                        */
                        ...
                        messageDiv.innerHTML='<div class="row" style="margin-left: -10px; margin-right: 0;">' +
                        '<div class="col-md-7"><strong>' + peer + '</strong></div>' +
                        '<div class="col-md-5 text-right"><small>' + curDate.getHours() + ':' + curDate.getMinutes() + '</small></div>' +
                        '</div>' +
                        '<div class="row" style="margin-left: -10px; margin-right: 0;">'+
                        '<div class="col-md-12">' + msgElement.outerHTML + '</div>'+
                        '</div>';
        ...
        _create: function() {
            ...
            uiChatbox = (self.uiChatbox = $('<div id=\"chat' + self.options.id + '\"></div>'))
                .appendTo(document.getElementById('chat_content'))
                // .addClass('ui-widget ' +
                //           'ui-corner-top ' +
                //           'ui-chatbox'
                //          )
            ...
            uiChatboxConf = (self.uiChatboxConf = $(
                '<div id=\'media\'>' +
                    '<div id=\'resizable\'>' +
                        '<video id="remoteVideo' + title + '" class="col-md-6" onDblClick="toggleFullScreen(' + title + ');" autoplay="autoplay" style="margin-top: 10px;-webkit-transition-property: opacity; -webkit-transition-duration: 2s; display:block; margin: 0 auto;" controls></video>' +
                    '</div>' +
                '</div>'))
                .addClass('ui-widget-content ui-chatbox-content')
                //.appendTo(uiChatbox),
                .appendTo(document.getElementById('rvContainer'));
            // titlebar
...
//                .text('minimize')
            ...
            // content
            uiChatboxContent = (self.uiChatboxContent = $('<div id="chatBoxContent'+ title + '"></div>'))
                //.addClass('ui-widget-content ui-chatbox-content')
                .appendTo(uiChatbox),
...
// incoming call
          ...
          uiChatboxIncomingCall = (self.uiChatboxIncomingCall = $('<span id="incomingCall' + title + '"></span>'))
                .html('&nbsp;<button id="acceptCall" onclick="mobicentsWebRTCPhoneController.onClickAcceptCallButtonViewEventHandler(true)"><img src="resources/images/video-camera-icon-green.png" width="25" height="25" style="font-size: 1.2em"/></button>&nbsp;<a id="acceptAudioCall" onclick="mobicentsWebRTCPhoneController.onClickAcceptCallButtonViewEventHandler(false)" href="#"><img src="resources/images/phone_pickup.png" width="25" height="25" style="font-size: 1.2em"/></a>&nbsp;<a id="rejectCall" onclick="mobicentsWebRTCPhoneController.onClickRejectCallButtonViewEventHandler()" href="#"><img src="resources/images/video-camera-icon-red.png" width="25" height="25" style="font-size: 1.2em"/></a>&nbsp;')
                .appendTo(uiChatboxContent),

        uiChatboxTitle = (self.uiChatboxTitle = $('<span id="commActions' + title + '"></span>'))
                .html('&nbsp;<a id="call'+title+'" href="#" onclick="mobicentsWebRTCPhoneController.onClickCallButtonViewEventHandler(\'' + title + '\',true)"><i class="icon-facetime-video" style="font-size: 1.2em;"/></a><a id="endCall' + title + '" href="#" onclick="mobicentsWebRTCPhoneController.onClickEndCallButtonViewEventHandler(true)"><span class="icon-stack"><i class="icon-facetime-video"></i><i class="icon-ban-circle icon-stack-base text-error"></i></span></a>&nbsp;<a id="audioCall'+title+'" href="#" onclick="mobicentsWebRTCPhoneController.onClickCallButtonViewEventHandler(\'' + title + '\',false)"><i class="icon-phone" style="font-size: 1.2em;"/></a><a id="endAudioCall' + title + '" href="#" onclick="mobicentsWebRTCPhoneController.onClickEndCallButtonViewEventHandler(false)"><span class="icon-stack"><i class="icon-phone"></i><i class="icon-ban-circle icon-stack-base text-error"></i></span></a>&nbsp;<a id="shareFile' + title + '" href="#" onclick="$(\'#fileShareModal\').modal()"><i class="icon-paper-clip"></i></a>&nbsp;<a id="dialPad' + title + '" href="#" onclick="$(\'#dialPadModal\').modal()"><i class="icon-th"></i></a>&nbsp;')
                .appendTo(uiChatboxContent),
       uiChatboxInput = (self.uiChatboxInput = $('<div style="margin: 1px; height: 20%;" id="chatInput' + title + '"></div>'))
                ...
                .appendTo(document.getElementById('chat')/*uiChatboxContent*/),
//
            uiChatboxInputBox = (self.uiChatboxInputBox = $('<textarea class="form-control" rows="1" style="height: 100%;border: none;box-shadow: none;border-top: 1px solid #DDD;border-radius: 0px;" placeholder="Send a Message..."></textarea>'))
                // .addClass('ui-widget-content ' +
...
var chatboxManager = function() {
 ...
 var toggleBox = function(id, toggle) {
    ...
    $('#participant_list > tbody:last').append(
        ...
        '<img class="img-circle" style="height:140px; width:140px;" src="resources/images/userPic_generic.png">' + id +
        '</td>' +
        '</tr>');
    if(toggle) {
