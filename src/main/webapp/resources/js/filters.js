...
var olyFilters = angular.module('mcWebRTC.filters', []);

olyFilters.filter('startFrom', function() {
...
olyFilters.filter('newlines', function () {
  return function(text) {
    return text !== null ? unescape(text).replace(/\\n/g, '\n').replace(/\\t/g, '\t').replace(/&/g, '\n').replace(/\+/g, ' ') : null;
  };
});

olyFilters.filter('noHTML', function () {
  return function(text) {
...
olyFilters.filter("timeago", function () {
  //time: the time
...
olyFilters.filter('bytes', function() {
